...
void Player::TileCollision(Tilemap* tilemap, float dt) {
	...
	for (int x = leftTile; x <= rightTile; x++) {
		for (int y = topTile; y <= bottomTile; y++) {
			Tileset::TileTypes tileType = tilemap->GetTile(x, y)->tileType;

			// check if the player is overlapping with a ladder
			if (tileType == Tileset::TileTypes::Ladder) {
			...
			if (tileType == Tileset::TileTypes::Collision && !climbing) {
				if (x == leftTile) {
	...
	if (velocity.y != 0) {
		...
		for (int x = leftTile; x <= rightTile; x++) {
			Tileset::TileTypes tileType = tilemap->GetTile(x, tileY)->tileType;
...
bool Player::AABBCollision(GameObject* object) {
	if (position.x + size.x >= object->GetPosition().x && position.x <= object->GetPosition().x + object->GetSize().x &&
		position.y + size.y >= object->GetPosition().y && position.y <= object->GetPosition().y + object->GetSize().y) {
		return true;
	}
	return false;
}
